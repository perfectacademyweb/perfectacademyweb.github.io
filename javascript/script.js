...
function isBangla(char) {
    ...
    function splitText(text) {
      ...
      for (let char of text) {
        let charClass = isBangla(char) ? 'bn-font' :
                        isEnglish(char) ? 'en-font' : '';

        if (charClass !== currentClass) {
          if (buffer) {
            result += currentClass ? `<span class="${currentClass}">${buffer}</span>` : buffer;
            buffer = '';
          }
    ...
    function processNode(node) {
      if (node.nodeType === Node.TEXT_NODE && node.textContent.trim() !== '') {
        ...
        span.innerHTML = splitText(node.textContent);
